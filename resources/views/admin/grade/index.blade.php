                ...
                <div class="card">
                    ...
                    <div class="card-content">
                        ...
                        <div class="card-body">
                            ...
                            @if(Auth()->user()->permissions->contains('name','admin.grade.create'))
                                <a href="{{ route('admin.grade.create') }}" class="btn btn-primary float-right"><i class="fa fa-plus"></i> Add</a>
                            @endif
                                ...
                                <table class="table table-striped dataex-html5-selectors_1">
                                    <thead>
                                        <tr>
                                            ...
                                            <th>Status</th>
                                            <th>User Type</th>
                                            @if( Auth()->user()->permissions->contains('name','admin.grade.edit') || Auth()->user()->permissions->contains('name','admin.grade.destroy') )
                                                <th>Action</th>
                                            @endif
                                        </tr>
                                    </thead>
                                        ...
                                        @if (count($gradeList) > 0)
                                            @foreach ($gradeList as $grade)
                                                <tr>
                                                    ...
                                                    <td>{{ $grade->name }}</td>
                                                    <td>{!! ($grade->status == '1') ? '<p class="badge badge-pill badge-light-primary">Active</p>' : '<p class="badge badge-pill badge-light-danger">Inactive</p>' !!}</td>
                                                    <td>{!! ($grade->userType == '1') ? '<p class="badge badge-pill badge-light-success">Buyer</p>' : '<p class="badge badge-pill badge-light-danger">Seller</p>' !!}</td>
                                                    @if( Auth()->user()->permissions->contains('name','admin.grade.edit') || Auth()->user()->permissions->contains('name','admin.grade.destroy') )
                                                        <td>
                                                            @if(Auth()->user()->permissions->contains('name','admin.grade.edit'))
                                                                <a href="{{ route('admin.grade.edit',$grade->id) }}"
                                                                    class="btn btn-primary btn-sm" title="Edit"><i
                                                                        class="fa fa-pencil fa-lg"></i>
                                                                </a>
                                                            @endif
                                                            @if(Auth()->user()->permissions->contains('name','admin.grade.destroy'))
                                                                <button type="button" class="btn btn-sm btn-danger delete-record"
                                                                    data-action="{{ route('admin.grade.destroy',$grade->id) }}">
