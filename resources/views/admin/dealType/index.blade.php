...
<div class="content-header row">
    ...
    <div class="content-header-left col-md-9 col-12 mb-2">
        <div class="row breadcrumbs-top">
            <div class="col-12">
                <h2 class="content-header-title float-left mb-0">Deal Type</h2>
                <div class="breadcrumb-wrapper col-12">
                    <ol class="breadcrumb">
                        <li class="breadcrumb-item">
                            <a href="{{ route('admin.dashboard') }}">Dashboard</a>
                        </li>
                        <li class="breadcrumb-item active">Deal Type
                        </li>
                    </ol>
                </div>
            </div>
            ...
            <div class="col-md-12">
                <div class="card">
                    <div class="card-header">
                        <h4 class="card-title">Deal Type List</h4>
                    </div>
                    <div class="card-content">
                        <div class="card-body">
                            <div class="button-container text-right mt-1 mb-1"></div>
                            @if(Auth()->user()->permissions->contains('name','admin.dealType.create'))
                                <a href="{{ route('admin.dealType.create') }}" class="btn btn-primary float-right"><i class="fa fa-plus"></i> Add</a>
                            @endif
                                ...
                                <table class="table table-striped dataex-html5-selectors_1">
                                    <thead>
                                        <tr>
                                            ...
                                            <th>Status</th>
                                            <th>User Type</th>
                                            @if( Auth()->user()->permissions->contains('name','admin.dealType.edit') || Auth()->user()->permissions->contains('name','admin.dealType.destroy') )
                                                <th>Action</th>
                                            @endif
                                        </tr>
                                    </thead>
                                    <tbody>
                                        @if (count($dealTypeList) > 0)
                                            @foreach ($dealTypeList as $dealType)
                                                <tr>
                                                    <td>{{ $loop->iteration }}</td>
                                                    <td>{{ $dealType->name }}</td>
                                                    <td>{!! ($dealType->status == '1') ? '<p class="badge badge-pill badge-light-primary">Active</p>' : '<p class="badge badge-pill badge-light-danger">Inactive</p>' !!}</td>
                                                     <td>{!! ($dealType->userType == '2') ? '<p class="badge badge-pill badge-light-success">Seller</p>' : '<p class="badge badge-pill badge-light-danger">Buyer</p>' !!}</td>
                                                    @if( Auth()->user()->permissions->contains('name','admin.dealType.edit') || Auth()->user()->permissions->contains('name','admin.dealType.destroy') )
                                                        <td>
                                                            @if(Auth()->user()->permissions->contains('name','admin.dealType.edit'))
                                                                <a href="{{ route('admin.dealType.edit',$dealType->id) }}"
                                                                    class="btn btn-primary btn-sm" title="Edit"><i
                                                                        class="fa fa-pencil fa-lg"></i>
                                                                </a>
                                                            @endif
                                                            @if(Auth()->user()->permissions->contains('name','admin.dealType.destroy'))
                                                                <button type="button" class="btn btn-sm btn-danger delete-record"
                                                                    data-action="{{ route('admin.dealType.destroy',$dealType->id) }}">
