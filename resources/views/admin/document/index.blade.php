...
<div class="content-header row">
    ...
    <div class="content-header-left col-md-9 col-12 mb-2">
        <div class="row breadcrumbs-top">
            <div class="col-12">
                <h2 class="content-header-title float-left mb-0">Document Master</h2>
                ...
                <div class="card">
                    ...
                    <div class="card-content">
                        ...
                        <div class="card-body">
                            ...
                            @if(Auth()->user()->permissions->contains('name','admin.document.create'))
                                <a href="{{ route('admin.document.create') }}" class="btn btn-primary float-right"><i class="fa fa-plus"></i> Add</a>
                            @endif
                                ...
                                <table class="table table-striped dataex-html5-selectors_1">
                                    <thead>
                                        <tr>
                                            ...
                                            <th>Status</th>
                                            <th>User Type</th>
                                            @if( Auth()->user()->permissions->contains('name','admin.document.edit') || Auth()->user()->permissions->contains('name','admin.document.destroy') )
                                                <th>Action</th>
                                            @endif
                                        </tr>
                                    </thead>
                                        ...
                                        @if (count($documentList) > 0)
                                            @foreach ($documentList as $document)
                                                <tr>
                                                    ...
                                                    <td>{{ $document->name }}</td>
                                                    <td>{!! ($document->status == '1') ? '<p class="badge badge-pill badge-light-primary">Active</p>' : '<p class="badge badge-pill badge-light-danger">Inactive</p>' !!}</td>
                                                      <td>{!! ($document->userType == '1') ? '<p class="badge badge-pill badge-light-success">Buyer</p>' : '<p class="badge badge-pill badge-light-danger">Seller</p>' !!}</td>
                                                    @if( Auth()->user()->permissions->contains('name','admin.document.edit') || Auth()->user()->permissions->contains('name','admin.document.destroy') )
                                                        <td>
                                                            @if(Auth()->user()->permissions->contains('name','admin.document.edit'))
                                                                <a href="{{ route('admin.document.edit',$document->id) }}"
                                                                    class="btn btn-primary btn-sm" title="Edit"><i
                                                                        class="fa fa-pencil fa-lg"></i>
                                                                </a>
                                                            @endif
                                                            @if(Auth()->user()->permissions->contains('name','admin.document.destroy'))
                                                                <button type="button" class="btn btn-sm btn-danger delete-record"
                                                                    data-action="{{ route('admin.document.destroy',$document->id) }}">
