...
<div class="content-header row">
    ...
    <div class="content-header-left col-md-9 col-12 mb-2">
        <div class="row breadcrumbs-top">
            <div class="col-12">
                <h2 class="content-header-title float-left mb-0">Colour</h2>
                <div class="breadcrumb-wrapper col-12">
                    <ol class="breadcrumb">
                        <li class="breadcrumb-item">
                            <a href="{{ route('admin.dashboard') }}">Dashboard</a>
                        </li>
                        <li class="breadcrumb-item active">Colours
                        </li>
                    </ol>
                </div>
            </div>
                ...
                <div class="card">
                    ...
                    <div class="card-content">
                        ...
                        <div class="card-body">
                            ...
                            @if(Auth()->user()->permissions->contains('name','admin.colour.create'))
                                <a href="{{ route('admin.colour.create') }}" class="btn btn-primary float-right"><i class="fa fa-plus"></i> Add</a>
                            @endif
                                ...
                                <table class="table table-striped dataex-html5-selectors_1">
                                    <thead>
                                        <tr>
                                            ...
                                            <th>Status</th>
                                            <th>User Type</th>
                                            @if( Auth()->user()->permissions->contains('name','admin.colour.edit') || Auth()->user()->permissions->contains('name','admin.colour.destroy') )
                                                <th>Action</th>
                                            @endif
                                        </tr>
                                    </thead>
                                    <tbody>
                                        @if (count($colourList) > 0)
                                            @foreach ($colourList as $colour)
                                                <tr>
                                                    <td>{{ $loop->iteration }}</td>
                                                    <td>{{ $colour->name }}</td>
                                                    <td>
                                                        <p class="p-1 border" style="background: {{ $colour->code }}"></p>
                                                    </td>
                                                    <td>{!! ($colour->status == '1') ? '<p class="badge badge-pill badge-light-primary">Active</p>' : '<p class="badge badge-pill badge-light-danger">Inactive</p>' !!}</td>
                                                    <td>{!! ($colour->userType == '1') ? '<p class="badge badge-pill badge-light-success">Buyer</p>' : '<p class="badge badge-pill badge-light-danger">Seller</p>' !!}</td>
                                                    @if( Auth()->user()->permissions->contains('name','admin.colour.edit') || Auth()->user()->permissions->contains('name','admin.colour.destroy') )
                                                        <td>
                                                            @if(Auth()->user()->permissions->contains('name','admin.colour.edit'))
                                                                <a href="{{ route('admin.colour.edit',$colour->id) }}"
                                                                    class="btn btn-primary btn-sm" title="Edit"><i
                                                                        class="fa fa-pencil fa-lg"></i>
                                                                </a>
                                                            @endif
                                                            @if(Auth()->user()->permissions->contains('name','admin.colour.destroy'))
                                                                <button type="button" class="btn btn-sm btn-danger delete-record"
                                                                    data-action="{{ route('admin.colour.destroy',$colour->id) }}">
