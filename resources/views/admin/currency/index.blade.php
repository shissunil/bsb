                ...
                <div class="card">
                    ...
                    <div class="card-content">
                        ...
                        <div class="card-body">
                            ...
                            @if(Auth()->user()->permissions->contains('name','admin.currency.create'))
                                <a href="{{ route('admin.currency.create') }}" class="btn btn-primary float-right"><i class="fa fa-plus"></i> Add</a>
                            @endif
                                ...
                                <table class="table table-striped dataex-html5-selectors_1">
                                    <thead>
                                        <tr>
                                            ...
                                            @if( Auth()->user()->permissions->contains('name','admin.currency.edit') || Auth()->user()->permissions->contains('name','admin.currency.destroy') )
                                                <th>Action</th>
                                            @endif
                                        </tr>
                                    </thead>
                                        ...
                                        @if (count($currencyList) > 0)
                                            @foreach ($currencyList as $currency)
                                                <tr>
                                                    ...
                                                    <td>{{ $currency->name }}</td>
                                                    <td>{!! ($currency->status == '1') ? '<p class="badge badge-pill badge-light-primary">Active</p>' : '<p class="badge badge-pill badge-light-danger">Inactive</p>' !!}</td>
                                                     <td>{!! ($currency->userType == '1') ? '<p class="badge badge-pill badge-light-success">Buyer</p>' : '<p class="badge badge-pill badge-light-danger">Seller</p>' !!}</td>
                                                    @if( Auth()->user()->permissions->contains('name','admin.currency.edit') || Auth()->user()->permissions->contains('name','admin.currency.destroy') )
                                                        <td>
                                                            @if(Auth()->user()->permissions->contains('name','admin.currency.edit'))
                                                                <a href="{{ route('admin.currency.edit',$currency->id) }}"
                                                                    class="btn btn-primary btn-sm" title="Edit"><i
                                                                        class="fa fa-pencil fa-lg"></i>
                                                                </a>
                                                            @endif
                                                            @if(Auth()->user()->permissions->contains('name','admin.currency.destroy'))
