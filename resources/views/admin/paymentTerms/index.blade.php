...
<div class="content-header row">
    ...
    <div class="content-header-left col-md-9 col-12 mb-2">
        <div class="row breadcrumbs-top">
            <div class="col-12">
                <h2 class="content-header-title float-left mb-0">Payment Terms</h2>
                ...
                <div class="card">
                    ...
                    <div class="card-content">
                        ...
                        <div class="card-body">
                            ...
                            @if(Auth()->user()->permissions->contains('name','admin.paymentTerms.create'))
                                <a href="{{ route('admin.paymentTerms.create') }}" class="btn btn-primary float-right"><i class="fa fa-plus"></i> Add</a>
                            @endif
                                ...
                                <table class="table table-striped dataex-html5-selectors_1">
                                    <thead>
                                        <tr>
                                            ...
                                            @if( Auth()->user()->permissions->contains('name','admin.paymentTerms.edit') || Auth()->user()->permissions->contains('name','admin.paymentTerms.destroy') )
                                                <th>Action</th>
                                            @endif
                                        </tr>
                                    </thead>
                                    <tbody>
                                        @if (count($paymentTermsList) > 0)
                                            @foreach ($paymentTermsList as $paymentTerms)
                                                <tr>
                                                    <td>{{ $loop->iteration }}</td>
                                                    <td>{{ $paymentTerms->name }}</td>
                                                    <td>{!! ($paymentTerms->status == '1') ? '<p class="badge badge-pill badge-light-primary">Active</p>' : '<p class="badge badge-pill badge-light-danger">Inactive</p>' !!}</td>
                                                    <td>{!! ($paymentTerms->userType == '1') ? '<p class="badge badge-pill badge-light-success">Buyer</p>' : '<p class="badge badge-pill badge-light-danger">Seller</p>' !!}</td>
                                                    @if( Auth()->user()->permissions->contains('name','admin.paymentTerms.edit') || Auth()->user()->permissions->contains('name','admin.paymentTerms.destroy') )
                                                        <td>
                                                            @if(Auth()->user()->permissions->contains('name','admin.paymentTerms.edit'))
                                                                <a href="{{ route('admin.paymentTerms.edit',$paymentTerms->id) }}"
                                                                    class="btn btn-primary btn-sm" title="Edit"><i
                                                                        class="fa fa-pencil fa-lg"></i>
                                                                </a>
                                                            @endif
                                                            @if(Auth()->user()->permissions->contains('name','admin.paymentTerms.destroy'))
                                                                <button type="button" class="btn btn-sm btn-danger delete-record"
                                                                    data-action="{{ route('admin.paymentTerms.destroy',$paymentTerms->id) }}">
