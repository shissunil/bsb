...
<div class="content-header row">
    ...
    <div class="content-header-left col-md-9 col-12 mb-2">
        <div class="row breadcrumbs-top">
            <div class="col-12">
                <h2 class="content-header-title float-left mb-0">Sub Admin Role</h2>
                <div class="breadcrumb-wrapper col-12">
                    <ol class="breadcrumb">
                        <li class="breadcrumb-item">
                            <a href="{{ route('admin.dashboard') }}">Dashboard</a>
                        </li>
                        <li class="breadcrumb-item active">Sub Admin Role
                        </li>
                    </ol>
                </div>
            </div>
            ...
            <div class="col-md-12">
                <div class="card">
                    <div class="card-header">
                        <h4 class="card-title">Sub Admin Role</h4>
                    </div>
                    <div class="card-content">

                        <div class="card-body">
                            <div class="button-container text-right mt-1 mb-1"></div>
                            @if(Auth()->user()->permissions->contains('name','admin.roles.create'))
                                <a href="{{ route('admin.roles.create') }}" class="btn btn-primary float-right"><i class="fa fa-plus"></i> Add</a>
                            @endif
                                ...
                                <table class="table table-striped dataex-html5-selectors_1">
                                    <thead>
                                        <tr>
                                            ...
                                            <th>Name</th>
                                            <th>Status</th>
                                            @if( Auth()->user()->permissions->contains('name','admin.roles.edit') || Auth()->user()->permissions->contains('name','admin.roles.destroy') )
                                                <th>Action</th>
                                            @endif
                                        </tr>
                                    </thead>
                                        ...
                                        @if (count($roleList) > 0)
                                            @foreach ($roleList as $role)
                                                <tr>
                                                    ...
                                                    <td>{{ $role->name }}</td>
                                                    <td>{!! ($role->status == '1') ? '<p class="badge badge-pill badge-light-primary">Active</p>' : '<p class="badge badge-pill badge-light-danger">Inactive</p>' !!}</td>
                                                    @if( Auth()->user()->permissions->contains('name','admin.roles.edit') || Auth()->user()->permissions->contains('name','admin.roles.destroy') )
                                                        <td>
                                                            @if(Auth()->user()->permissions->contains('name','admin.roles.edit'))
                                                                <a href="{{ route('admin.roles.edit',$role->id) }}"
                                                                    class="btn btn-primary btn-sm" title="Edit"><i
                                                                        class="fa fa-pencil fa-lg"></i>
                                                                </a>
                                                            @endif
                                                            @if(Auth()->user()->permissions->contains('name','admin.roles.destroy'))
