            ...
            <div class="col-12">
                ...
                <div class="breadcrumb-wrapper col-12">
                    <ol class="breadcrumb">
                        ...
                        <li class="breadcrumb-item active">User Type
                        </li>
                    </ol>
                </div>
            </div>
                ...
                <div class="card">
                    ...
                    <div class="card-content">
                        ...
                        <div class="card-body">
                            ...
                            @if(Auth()->user()->permissions->contains('name','admin.userType.create'))
                                <a href="{{ route('admin.userType.create') }}" class="btn btn-primary float-right"><i class="fa fa-plus"></i> Add</a>
                            @endif
                                ...
                                <table class="table table-striped dataex-html5-selectors_1">
                                    <thead>
                                        <tr>
                                            ...
                                            <th>Status</th>
                                            <th>User Type</th>
                                            @if( Auth()->user()->permissions->contains('name','admin.userType.edit') || Auth()->user()->permissions->contains('name','admin.userType.destroy') )
                                                <th>Action</th>
                                            @endif
                                        </tr>
                                    </thead>
                                        ...
                                        @if (count($userTypeList) > 0)
                                            @foreach ($userTypeList as $userType)
                                                <tr>
                                                    ...
                                                    <td>{{ $userType->name }}</td>
                                                    <td>{!! ($userType->status == '1') ? '<p class="badge badge-pill badge-light-primary">Active</p>' : '<p class="badge badge-pill badge-light-danger">Inactive</p>' !!}</td>
                                                    <td>{!! ($userType->userType == '2') ? '<p class="badge badge-pill badge-light-success">Seller</p>' : '<p class="badge badge-pill badge-light-danger">Buyer</p>' !!}</td>
                                                    @if( Auth()->user()->permissions->contains('id','admin.userType.edit') || Auth()->user()->permissions->contains('name','admin.userType.destroy') )
                                                        <td>
                                                            @if(Auth()->user()->permissions->contains('name','admin.userType.edit'))
                                                                <a href="{{ route('admin.userType.edit',$userType->id) }}"
                                                                    class="btn btn-primary btn-sm" title="Edit"><i
                                                                        class="fa fa-pencil fa-lg"></i>
                                                                </a>
                                                            @endif
                                                            @if(Auth()->user()->permissions->contains('name','admin.userType.destroy'))
                                                                <button type="button" class="btn btn-sm btn-danger delete-record"
                                                                    data-action="{{ route('admin.userType.destroy',$userType->id) }}">
